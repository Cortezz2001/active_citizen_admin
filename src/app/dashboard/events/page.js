...
import { searchEvents, filterEvents } from './eventsService';
...
export default function EventsPage() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        const eventsData = await getData('events', cityKey);
  ...
  // Filter and sort events based on search query and filter parameters
  const filteredEvents = useMemo(() => {
    let filtered = searchEvents(events, searchQuery);
    filtered = filterEvents(filtered, filterParams);
  ...
  }, [events, searchQuery, filterParams]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsDeleting(true);
      const cityKey = localStorage.getItem('selectedCity') || '';
      await deleteItem('events', itemToDelete, cityKey);
      ...
      const newTotalItems = filteredEvents.length - 1;
    ...
    <div className="space-y-4 font-mregular">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center">
          <h1 className="text-2xl font-mbold">События</h1>
          {isSearchActive && (
      ...
      <form onSubmit={handleSearchSubmit} className="flex items-center mb-6">
        ...
        <button
          ...
          className="ml-4 px-4 py-2 rounded-lg font-msemibold transition-colors bg-light-surface dark:bg-dark-surface border border-light-border dark:border-dark-dark-border hover:bg-light-border dark:hover:bg-dark-border"
        >
        ...
        <div className="flex justify-between items-center text-sm text-light-text-secondary dark:text-dark-text-secondary font-mmedium">
          <span className='font-mregular'>
            {searchQuery || filterParams.status !== 'all' || filterParams.categoryId !== 'all' || filterParams.dateFrom || filterParams.dateTo || filterParams.sortOrder !== 'newest' ? (
              <>
                Найдено {filteredEvents.length} из {events.length} записей
                {filteredEvents.length > itemsPerPage && (
                  <span className="ml-2">
                    (показано {startIndex + 1}-{Math.min(endIndex, filteredEvents.length)})
                  </span>
                )}
              </>
            ) : (
              <>
                Показано {startIndex + 1}-{Math.min(endIndex, filteredEvents.length)} из {filteredEvents.length} записей
              </>
            )}
          </span>
          {totalPages > 1 && (
        ...
        >
          ...
          <div className="flex-1 p-4 flex flex-col">
            ...
            <div className="flex items-center text-sm text-light-text-secondary dark:text-dark-text-secondary mb-3 font-mmedium">
              ...
              <span>
                Дата создания: {item.created_at?.toDate().toLocaleDateString('ru-RU') || 'не указана'}
              </span>
            </div>
      ...
      {(searchQuery || filterParams.status !== 'all' || filterParams.categoryId !== 'all' || filterParams.dateFrom || filterParams.dateTo || filterParams.sortOrder !== 'newest') && filteredEvents.length === 0 && events.length > 0 && (
